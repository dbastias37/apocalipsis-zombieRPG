...
export default function StockFoodModal({ isOpen, onClose, state, setState }: Props){
  ...
  const confirm = ()=>{
    const { state: st, taken } = takeFoodFromCamp(state, amount);
    const stash = Array.isArray(st?.camp?.stash) ? [...st.camp.stash] : [];
    if(taken>0){
      const item: FoodItem = { type:'ration', food:taken };
      stash.push(item);
    }
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 flex items-center justify-center" style={{zIndex:9999}}>
      ...
      <div className="relative z-10 w-full max-w-sm p-6 rounded-2xl bg-neutral-900 border border-neutral-800 text-neutral-100" role="dialog" aria-modal>
        ...
        <h3 className="text-lg font-bold mb-2">Stock de Alimentos</h3>
        <p className="text-sm text-neutral-400 mb-4">Stock: {stock} raciones</p>
        <div className="space-y-4">
          ...
          <div className="text-sm text-neutral-400">Retirarás {amount} raciones</div>
        </div>
