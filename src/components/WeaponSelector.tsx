...
export default function WeaponSelector({
  player, weapons, value, onChange, disabled
}: { player:any; weapons: Weapon[]; value?: string; onChange:(id:string)=>void; disabled?: boolean }) {
  return (
    <div className="flex flex-wrap gap-2">
      {weapons.map(w => {
        ...
        const title = noAmmo ? "No hay munición." : `${w.name} • +${w.hitBonus} golpe • ${w.damage.times}d${w.damage.faces}${w.damage.mod>=0?`+${w.damage.mod}`:w.damage.mod}`;
        return (
          <button
            key={w.id}
            disabled={disabled}
